...
function getElements(fixtureNativeElement: any, selector: string) {
  return fixtureNativeElement.querySelectorAll(selector) as HTMLElement;
}
  ...
  */
  ...
  xdescribe('validate arrowkeys', () => {
    ...
    it('should can change input value with the arrow keys', () => {
      const hourSpy = spyOn(component, 'changeHours').and.callThrough();
      component.hourStep = 3;
      ...
      fixture.whenStable().then(() => {
        ...
        expect(hourSpy).toHaveBeenCalledWith(component.hourStep, 'key');
      });
    });
